...
(function () {
   ...
   */
  g.STORYMODE_PROMPT_ASSET_VERSION = 202604303;
  ...
  /** Pipeline โรงงานแยดจาก Story/Product — ผลเป็น factory_output_v2 เท่านั้น */
  const FACTORY_OUTPUT_SCHEMA_ID = 'factory_output_v2';
  ...
  function buildFactoryWarehouseSystemPromptFromPayload(payload, opts) {
    ...
    const visualId =
      (payload.visualStyleIds && payload.visualStyleIds[0]) || (vObj && vObj.id) || 'real_cinematic';
    const smVisualStyleDisplay = (vObj && vObj.name) || String(visualId);
    const visualDesc =
      visualStyleEngMap[visualId] ||
      visualStyleEngMap.real_cinematic ||
      'Photorealistic cinematic style, natural lighting, high detail.';
    ...
    const voiceGenderBanTh =
      resolvedVoice.gender === 'male'
        ? 'ห้ามใช้เสียงผู้หญิงเด็ดขาด'
        : 'ห้ามใช้เสียงผู้ชายเด็ดขาด';

    const characterCardResult =
      typeof buildCompactCharacterCard === 'function'
        ? buildCompactCharacterCard(payload, resolvedVoice)
        : null;

    const outputTypeNote =
      smOutputType === 'image'
        ? 'สร้างเฉพาะ 🔴 IMAGE prompt เท่านั้น (ไม่ต้องมี VIDEO)'
        : smOutputType === 'video'
          ? 'สร้างเฉพาะ 🟢 VIDEO prompt เท่านั้น (ไม่ต้องมี IMAGE)'
          : 'สร้างทั้ง 🔴 IMAGE และ 🟢 VIDEO ในทุกฉาก';
    ...
    if (moods.length > 1) {
      moodDirectiveEn +=
        '\n\nAdditional mood labels to blend: ' +
        moods
          .slice(1)
          .map(function (m) {
            return '"' + m + '"';
    ...
    if (narrativeBlock) {
      narrativeSection =
        '═══ NARRATIVE PERSONA (EN — optional cues) ═══\n' + narrativeBlock + '\n';
    }

    const voiceOnlyNoMusicSfxBlock =
      '═══ AUDIO — Thai voice-only ═══\n' +
      'ห้าม background music / SFX แยกจากเสียงพูดใน prompt — เหลือเสียงพูดไทยเป็นหลัก\n' +
      'ท้ายแต่ละ 🟢 VIDEO ต้องมีบรรทัดสรุป: **Audio: Thai voice-only. No music. No SFX.**\n\n';

    const imgTpl =
      visualDesc +
      '. Warehouse / bulk-stock TikTok aesthetic per FACTORY DNA: [SCENE_DESCRIPTION]. [CAMERA]. Handheld or steady per DNA pacing. No collage. Single frame.';
    const vidTpl =
      'ACTION: [CHARACTER_ACTION]. Camera per DNA. Lip-sync Thai or warehouse narrator per Speaker line. Thai dialogue locked in Dialogue line. NO subtitles. AUDIO ONLY. End block with Speaker:, Dialogue:, then Audio summary line.';

    return (
      'คุณคือ Creative Director โหมด **โกดัง / warehouse bulk-sale TikTok** — pipeline แยดจาก Story mode และ Product sell blueprint\n' +
      'แม่แบบหลักคือ JSON **FACTORY DNA** ใน user message (schema factory_dna_v2 หรือ v1) — โคลนโทน จังหวะ และภาษาขายจากแพ็กโรงงาน\n\n' +
      '⛔ **ห้าม** ใช้หัวข้อหรือรูปแบบผลลัพธ์แบบ Story mode เช่น `=== SCENE N:` หรือ blueprint Sales Formula — โปรเจกต์นี้แยก state โรงงานชัดเจน\n\n' +
      '═══ VISUAL STYLE (บังคับ) ═══\n' +
      ...
      narrativeSection +
      '═══ OUTPUT FORMAT — ' +
      FACTORY_OUTPUT_SCHEMA_ID +
      ' (บังคับเป๊ะ) ═══\n\n' +
      'เปิดด้วยบรรทัดเดียว:\n' +
      '`BEGIN_FACTORY_OUTPUT_V2`\n\n' +
      'จากนั้น **แต่ละฉาก** ใช้หัวข้อนี้เท่านั้น (ไม่ใช้ SCENE ของ Story):\n' +
      '`=== FACTORY_SCENE [N]: [SHORT_EN_NAME] ===`\n\n' +
      outputTypeNote +
      '\n\n' +
      'ถ้าสร้างภาพ — ใส่บล็อก:\n' +
      '🔴 IMAGE_PROMPT_EN\n' +
      '```\n' +
      '[ภาษาอังกฤษเท่านั้น — ' +
      imgTpl.replace(/\n/g, ' ') +
      ']\n' +
      '```\n\n' +
      'ถ้าสร้างวิดีโอ — ใส่บล็อก:\n' +
      '🟢 VIDEO_PROMPT_EN\n' +
      '```\n' +
      '[ภาษาอังกฤษ — ' +
      vidTpl.replace(/\n/g, ' ') +
      ']\n' +
      'Speaker: (ROLE_warehouse_hero หรือฉลากจาก DNA — หนึ่งผู้พูดหลักต่อฉาก)\n' +
      'Dialogue: "บทไทย 15-20 คำ"\n' +
      'Audio: Thai voice-only (Speaker id). No music. No SFX.\n' +
      '```\n\n' +
      'ท้ายสุดหลังฉากสุดท้าย:\n' +
      '`### VIRAL_CAPTION_TH`\n' +
      '"แคปชั่นไทยสั้นๆ โทนโกดัง"\n' +
      '`### HASHTAGS`\n' +
      '#แท็ก1 #แท็ก2 #แท็ก3\n' +
      '`END_FACTORY_OUTPUT_V2`\n\n' +
      '═══ IMAGE PROMPT MICRO-TEMPLATE (EN) ═══\n' +
      imgTpl +
      '\n\n' +
      '═══ VIDEO PROMPT MICRO-TEMPLATE (EN) ═══\n' +
      vidTpl +
      '\n\n' +
      buildDialogueWordBudgetThai(false, 'system') +
      voiceOnlyNoMusicSfxBlock +
      '═══ CRITICAL RULES (FACTORY) ═══\n' +
      '1. ผลลัพธ์ต้องอยู่ในกรอบ BEGIN/END และหัว FACTORY_SCENE เท่านั้น\n' +
      '2. Image prompt EN — Video prompt EN — Dialogue ไทยใน Dialogue: เท่านั้น\n' +
      '3. จำนวนฉาก: ' +
      smSceneCount +
      ' — โฟกัสโกดัง พื้นแพ็กของกอง สต็อก ช็อตมือถือ ตาม DNA\n' +
      '4. ห้าม overclaim / คำต้องห้ามโฆษณา — เหมือน Story safety\n' +
      '5. HERO — ล็อกความสม่ำเสมอจาก HERO BIBLE ด้านล่างเมื่อมีตัวละคร\n' +
      '6. Voice hint: ' +
      voiceEn +
      ' — ' +
      voiceGenderBanTh +
  ...
  function buildFactoryWarehouseUserMessageFromPayload(payload, opts) {
    opts = opts || {};
    const topic = sanitizeStorymodeUserPlainText((payload.prompt || '').trim());
    if (!topic) return '';

    const smStoryType = opts.storyType || 'custom';
    const storyTypeObj = STORY_TYPE_TEMPLATES.find(function (t) {
      return t.id === smStoryType;
    });
    const storyTypeLabel = storyTypeObj
      ? storyTypeObj.name + ' — ' + storyTypeObj.description
      : smStoryType;
    ...
    var msg =
      '═══ FACTORY MODE — บรีฟหลัก (pipeline แยดจาก Story / Product blueprint) ═══\n' +
      topic +
      '\n';
    ...
    msg += buildDialogueWordBudgetThai(false, 'user');
    ...
    if (smOutputType !== 'both') {
      msg +=
        '\n═══ ประเภท Output ═══\n' +
        (smOutputType === 'image' ? 'สร้างเฉพาะรูป' : 'สร้างเฉพาะวิดีโอ') +
        '\n';
    }

    if (vObj && vObj.prompt) {
      msg +=
        '\n═══ Visual style directive (EN) ═══\n' +
        vObj.prompt +
        '\n';
    }
    ...
    msg +=
      ...
      (payload.textH1H2Enabled
        ? 'ต้องการข้อความบนภาพฉากแรกถ้าเหมาะสม'
        : 'ไม่บังคับ H1/H2 บนภาพ') +
      '\n';

    const img = payload.images || {};
    const charNames =
      img.characterNames && img.characterNames.length
        ? img.characterNames
        : img.characterName
          ? [img.characterName]
          : [];
    const productNames =
      img.productNames && img.productNames.length
        ? img.productNames
        : img.productName
          ? [img.productName]
          : [];
    if (img.productAttached || img.characterAttached) {
      ...
      if (img.productAttached) {
        msg +=
          'สินค้า: ' +
          (img.productAttachedCount || productNames.length || 1) +
          ' รูป — ' +
          (productNames.join(', ') || '(attached)') +
          '\n';
      }
      if (img.characterAttached) {
        var ccounts = img.characterImageCounts;
        if (ccounts && ccounts.length) {
          msg += 'ตัวละคร ref ต่อ slot: ' + ccounts.join(' / ') + '\n';
        }
        msg += 'ชื่อไฟล์ ref: ' + (charNames.join(', ') || '(attached)') + '\n';
      }
    }
  ...
  window.MockStorymodeGemini = {
    ...
    FACTORY_DNA_V2_SCHEMA_VERSION: 'factory_dna_v2',
    FACTORY_OUTPUT_SCHEMA_ID: FACTORY_OUTPUT_SCHEMA_ID,
    STORYMODE_PROMPT_ASSET_VERSION: g.STORYMODE_PROMPT_ASSET_VERSION,
    STORY_TYPE_TEMPLATES: STORY_TYPE_TEMPLATES,
    buildFactoryWarehouseSystemPromptFromPayload: buildFactoryWarehouseSystemPromptFromPayload,
    buildFactoryWarehouseUserMessageFromPayload: buildFactoryWarehouseUserMessageFromPayload,
